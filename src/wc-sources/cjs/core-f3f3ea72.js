...
const patchBrowser = async () => {
    // @ts-ignore
    const importMeta = (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('core-f3f3ea72.js', document.baseURI).href));
    ...
    if ( (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if ( !Cstr.$isStyleRegistered$ && Cstr.style) {
            ...
            if ( (shadowCssPolyfill || runtimeScopedCss)) {
                style = await new Promise(function (resolve) { resolve(require('./shadow-css-9e778f69-fbbb5254.js')); }).then(m => m.scopeCss(style, scopeId, false));
            }
